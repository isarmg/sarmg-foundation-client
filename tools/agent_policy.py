...
AGENT_LIMIT_KEYS = {"max_record_bytes", "max_spool_bytes", "max_spool_entries"}
...
def load_profiles(foundation_root: Path) -> tuple[dict[str, dict[str, Any]], set[str]]:
    ...
    for path in sorted(profile_root.glob("*.toml")):
        ...
        if "bounded-spool" in required:
            ...
            if not isinstance(policy, dict) or not isinstance(policy.get("agent_limits"), dict):
                raise ConformanceError(f"{path}: bounded-spool requires policy.agent_limits")
            limits = policy["agent_limits"]
            _exact_keys(limits, AGENT_LIMIT_KEYS, AGENT_LIMIT_KEYS, f"{path}.policy.agent_limits")
            if any(type(limit) is not int or limit <= 0 for limit in limits.values()):
                raise ConformanceError(f"{path}: agent limits must be positive integers")
    ...
    if set(profiles) != {"desktop-agent", "mobile-agent"}:
...
def load_product_manifest(product_root: Path) -> dict[str, Any]:
    return _toml(product_root / "sarmg-agent.toml")
...
def verify_manifest(product_root: Path, foundation_root: Path) -> dict[str, Any]:
    ...
    for index, component in enumerate(components):
        ...
        _exact_keys(
            component,
            {"id", "profile", "http_adapter", "web_profile", "capabilities", "agent_limits"},
        ...
        if "bounded-spool" in declared:
            limits = component.get("agent_limits")
            if not isinstance(limits, dict):
                raise ConformanceError(f"{context}: bounded-spool requires agent_limits")
            _exact_keys(limits, AGENT_LIMIT_KEYS, AGENT_LIMIT_KEYS, f"{context}.agent_limits")
            maximum = profile["policy"]["agent_limits"]
            if any(type(limit) is not int or not 0 < limit <= maximum[key] for key, limit in limits.items()):
                raise ConformanceError(f"{context}: agent_limits exceed the Foundation Profile")
        elif "agent_limits" in component:
            raise ConformanceError(f"{context}: Profile does not accept agent_limits")
...
def verify_source(product_root: Path, foundation_root: Path) -> dict[str, Any]:
    ...
    if "bounded-spool" in capabilities:
        patterns.append(("agent-runtime-ownership", re.compile(
            r"\bfn\s+(?:jitter|sampling_jitter|retry_jitter|exponential_backoff)\s*\("
            r"|\bSARMGSPOOL\b|\bspool\.instance\.lock\b"
            r"|\b(?:enum\s+(?:FlushOutcome|FailureDisposition|QuarantineReason|CredentialAuthorization|CredentialMutation)|struct\s+(?:RetryBackoff|DeliveryWorker|DeliveryTrigger|QueueFailureStreak|CredentialSnapshot|AgentIdentity|AgentSession|SingleInstanceLock)|trait\s+CredentialStore|fn\s+deliver_batch)\b"
        ...
        for directory, directories, files in os.walk(root, followlinks=False):
            ...
            for name in sorted(files):
                ...
                if name == "package.json":
                    ...
                    for section in ("dependencies", "devDependencies", "optionalDependencies", "peerDependencies"):
                        for dependency in package_json.get(section, {}):
                            if dependency.startswith("@sarmg/") and not dependency.startswith("@sarmg/agent-"):
                ...
                for dependency, requirement in _walk_dependencies(_toml(path)):
                    ...
                    if package.startswith("sarmg-") and not (package.startswith("sarmg-agent-") or package == "sarmg-mobile-ffi"):
