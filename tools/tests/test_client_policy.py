...
class ClientPolicyTests(unittest.TestCase):
    ...
    def test_server_profiles_and_dependencies_are_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            manifest = root / "sarmg-client.toml"
    ...
    def test_client_web_stays_in_client_policy(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "sarmg-client.toml").write_text(VALID_MANIFEST + component)
            source = root / "app.js"
...
'''
        with tempfile.TemporaryDirectory() as directory:
            product = Path(directory)
            path = product / "sarmg-client.toml"
...
'''
        with tempfile.TemporaryDirectory() as directory:
            product = Path(directory)
            (product / "sarmg-client.toml").write_text(manifest, encoding="utf-8")
            ...
            for text in [
                ...
                with self.subTest(text=text):
                    ...
                    with self.assertRaisesRegex(ConformanceError, "client-runtime-ownership"):
                        verify_source(product, ROOT)
            source.write_text(
                "use sarmg_client_runtime::retry_jitter;\n"
