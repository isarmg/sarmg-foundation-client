...
CLI 输出经过递归脱敏：字段名大小写和连接符不影响匹配，密码、令牌、凭据、证书及 API/私钥字段中的字符串、数字、对象或数组会整体替换为 `{"configured": boolean}`；布尔值保留，供状态标志使用。普通字符串和人类可读输出中的字段名会移除控制字符。产品应使用明确的字段名表达秘密，并且不得将秘密放入普通字段或错误详情中。
