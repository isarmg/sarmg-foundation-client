...
`sarmg-agent-runtime::AgentIdentity` is the immutable runtime identity of an
Agent delivery context: product ID, instance ID and current payload contract.
...
Host's `agent_identity` adapter keeps the canonical lowercase hyphenated UUID
