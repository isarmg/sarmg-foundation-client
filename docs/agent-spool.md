...
`desktop-agent` declares hard ceilings of 1 MiB payload, 256 MiB spool and 4096
...
`DeliveryWorker<AgentDeliveryDriver>` owns the delivery/recovery loop, coalescing
...
`AgentSession` owns an anchored private state directory and `SingleInstanceLock`
on `agent.instance.lock`. Acquire it before delivery bootstrap or collection and
...
directory when creating the child queue and retains an `Arc<AgentSession>`;
