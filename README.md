...
当前源码版本为 `0.9.13`，统一采用 Client 命名；未通过对应原生 CI 前，不宣称跨平台验收完成。
...
| `sarmg-client-cli` | 产品中立的参数、输出信封、受保护终端输入和系统服务生命周期；产品错误目录由 Adapter 提供 |
| `sarmg-secure-xml` | 产品中立的有界 XML 解析，拒绝 DTD/ENTITY 并限制深度、节点、文本和时间；命名空间与字段语义由产品负责 |
