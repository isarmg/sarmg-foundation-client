...
impl Spool {
    ...
    pub fn inspect_existing(
        ...
        limits: SpoolLimits,
    ) -> Result<crate::ClientHealth, Error> {
        limits.validate()?;
    ...
    pub fn next(&self) -> Result<Option<SpoolRecord>, Error> {
        ...
        for entry in self.entries()? {
            ...
            let bytes = match self.directory.read_bounded(
                ...
                Ok(bytes) => bytes,
                Err(sarmg_client_fs_safety::Error::BudgetExceeded) => {
    ...
    pub fn doctor(&self) -> Result<crate::ClientHealth, Error> {
        let _guard = self.guard()?;
...
mod tests {
    ...
    fn inspection_is_read_only_and_works_while_the_writer_holds_its_lock() {
        ...
        assert!(matches!(
            ...
            Err(Error::Filesystem(sarmg_client_fs_safety::Error::Io(error)))
    ...
    fn runtime_limits_match_the_checked_profile() {
        let profile = include_str!("../../../../profiles/desktop-client.toml");
        let (_, policy) = profile.split_once("[policy.client_limits]").unwrap();
        let declared = policy
            .lines()
