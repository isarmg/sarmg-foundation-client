...
use rand::Rng;
use sarmg_client_fs_safety::{AdvisoryLock, PrivateDirectory, RelativePath};
...
pub use worker::{
    ClientDeliveryDriver, DeliveryFuture, DeliveryNotifications, DeliveryResponse, DeliveryWake,
...
impl SingleInstanceLock {
    pub fn acquire(state_directory: &PrivateDirectory) -> Result<Self, Error> {
        let name = RelativePath::new("client.instance.lock")?;
        let lock = AdvisoryLock::acquire(state_directory, &name).map_err(|error| {
            if matches!(error, sarmg_client_fs_safety::Error::AlreadyLocked(_)) {
...
/// Jitter for a validated Client sampling interval. Network retries must use
...
pub enum Error {
    ...
    InvalidIdentity,
    #[error("Client identity does not match the active delivery identity")]
    IdentityMismatch,
    #[error("another client instance already owns the state directory")]
    AlreadyRunning,
    #[error("persistent local queue failure requires service intervention")]
    PersistentQueueFailure,
    #[error(transparent)]
    Filesystem(#[from] sarmg_client_fs_safety::Error),
    #[error(transparent)]
