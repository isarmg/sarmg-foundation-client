...
use sarmg_client_fs_safety::PrivateDirectory;
use std::path::Path;
...
mod tests {
    ...
    fn session_rejects_unsafe_lock_without_repair() {
        ...
        let lock = path.join("client.instance.lock");
        symlink(&target, &lock).unwrap();
        assert!(ClientSession::open(&path).is_err());
    ...
    fn session_excludes_another_process_until_its_owner_drops() {
        ...
        let check_contender = |expected| {
            let result = Command::new(std::env::current_exe().unwrap())
                ...
                .env("SARMG_CLIENT_SESSION_TEST_PATH", &path)
                .env("SARMG_CLIENT_SESSION_TEST_EXPECT", expected)
                .output()
                .unwrap();
            assert!(
        ...
        assert!(path.join("client.instance.lock").is_file());
    }

    #[test]
    #[ignore = "subprocess helper invoked by session_excludes_another_process_until_its_owner_drops"]
    fn contender_process_helper() {
        let path = std::env::var_os("SARMG_CLIENT_SESSION_TEST_PATH").unwrap();
        let result = ClientSession::open(Path::new(&path));
        if std::env::var("SARMG_CLIENT_SESSION_TEST_EXPECT").unwrap() == "busy" {
