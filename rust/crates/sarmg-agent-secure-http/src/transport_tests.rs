...
fn loopback_http_tracks_build_mode() {
    ...
    assert_eq!(agent_network_policy(&url).is_ok(), cfg!(debug_assertions));
    assert_eq!(
        validate_url_structure(NetworkPolicy::LoopbackDevelopment, &url).is_ok(),
        cfg!(debug_assertions)
    );
    assert!(agent_network_policy(&Url::parse("https://127.0.0.1:8080/").unwrap()).is_ok());
    assert!(agent_network_policy(&Url::parse("http://192.0.2.1/").unwrap()).is_err());
    #[cfg(not(debug_assertions))]
    {
        let transport = client(1024, 1024, Duration::from_secs(1));
        assert!(matches!(
            transport.get_agent_blocking(url.as_str(), header::HeaderMap::new()),
        ...
        let runtime = tokio::runtime::Builder::new_current_thread()
            ...
            .unwrap();
        assert!(matches!(
            runtime.block_on(transport.post_agent(
                url.as_str(),
...
async fn bounded_execution_checks_headers_content_length_and_chunked_body() {
    for (response, body_budget, header_budget, accepted) in [
        ...
        let result = client(body_budget, header_budget, Duration::from_secs(2))
            .post_agent(&url, header::HeaderMap::new(), Vec::new())
            .await;
        worker.join().unwrap();
        if accepted {
...
async fn redirects_do_not_replay_requests_or_credentials() {
    ...
    let response = client(1024, 1024, Duration::from_secs(2))
        .post_agent(&url, headers, Vec::new())
        .await
        .unwrap();
...
async fn blocking_adapter_can_run_inside_tokio_and_uses_the_bounded_get_path() {
    ...
    let result = client(4, 1024, Duration::from_secs(3))
        .get_agent_blocking(&url, header::HeaderMap::new())
        .unwrap();
    ...
    let result =
        client(4, 1024, Duration::from_secs(3)).get_agent_blocking(&url, header::HeaderMap::new());
    worker.join().unwrap();
...
async fn total_timeout_includes_body_reading_and_errors_do_not_keep_sources() {
    ...
    let result = tokio::time::timeout(
        ...
        transport.post_agent(&url, header::HeaderMap::new(), Vec::new()),
    )
    .await;
    release.send(()).unwrap();
    let request = worker.join().unwrap();
