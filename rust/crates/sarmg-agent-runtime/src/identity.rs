...
mod tests {
    use super::*;
    fn identity(product: &str, instance: &str, contract: &str) -> AgentIdentity {
        AgentIdentity::new(product, instance, ContractId::new(contract).unwrap()).unwrap()
    }
    ...
    fn identity_is_bounded_and_never_normalizes_untrusted_identifiers() {
        ...
        for invalid in [
            ...
            for (product, instance) in [(invalid, "instance"), ("product", invalid)] {
                assert!(matches!(
                    AgentIdentity::new(product, instance, ContractId::new("current").unwrap()),
                    Err(Error::InvalidIdentity)
                ));
            }
        }
        assert!(
            AgentIdentity::new("p".repeat(129), "i", ContractId::new("current").unwrap()).is_err()
        );
        assert!(
            AgentIdentity::new("p", "i".repeat(257), ContractId::new("current").unwrap()).is_err()
        );
    }
